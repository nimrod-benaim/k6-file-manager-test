...
export default function () {
  ...
  const loginPage = http.get(`${config.baseUrl}${config.endpoints.login}`);
  ...
  const loginRes = http.post(`${config.baseUrl}${config.endpoints.loginPost}`,payload,{ headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
  loginDuration.add(loginRes.timings.duration);
  check(loginRes, { 'Login successful': (r) => r.status === 200 || r.status === 302 });
  console.log('Login response code:', loginRes.status);

  // Step 4: Get JWT Token
  const jwtRes = http.get(`${config.baseUrl}${config.endpoints.jwt}`);
  ...
  const fmRes = http.get(`${config.baseUrl}${config.endpoints.fileManager}`);//mabey add cookies
  ...
  const searchRes = http.get(searchUrl, {headers: {Authorization: jwtToken, 'Content-Type': 'application/json'}});
